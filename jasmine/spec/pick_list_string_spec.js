describe("Pick List (String)", function () {

    beforeEach(function () {
        loadFixtures('string_data.html');

        expect($('#languages')).toBeVisible();
        $('#languages').pickList({sortItems: false, useData: true});
    ...
    it("should populate source list", function () {
        ...
        expect($(options.get(2)).text()).toBe('Scala');
        expect($(options.get(2)).attr('value')).toBe(3);

        expect($(options.get(3)).text()).toBe('Ruby');
